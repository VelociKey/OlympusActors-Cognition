...
func (s *CoderServer) ExecuteTask(ctx context.Context, req *connect.Request[olympusv1.TaskRequest]) (*connect.Response[olympusv1.TaskResponse], error) {
	...
	if err != nil { return nil, connect.NewError(connect.CodePermissionDenied, err) }
	...
	status := "SUCCESS"; if nlErr != nil { status = "FAILURE" }
...
func (s *CoderServer) ExecuteMission(ctx context.Context, req *connect.Request[olympusv1.MissionRequest]) (*connect.Response[olympusv1.MissionResponse], error) {
	...
	data, err := os.ReadFile(fullPath); if err != nil { return nil, connect.NewError(connect.CodeNotFound, err) }
...
func (s *CoderServer) commit(path string) error {
	...
	cmdAdd.Dir = path
	if err := cmdAdd.Run(); err != nil { return err }
...
func main() {
	...
	mux := http.NewServeMux(); mux.Handle(olympusv1connect.NewCoderServiceHandler(server, interceptors)); mux.Handle(olympusv1connect.NewMissionServiceHandler(server, interceptors))
	srv := &http.Server{
		Addr:              ":8083",
...
func validateSandbox(workspace string) (string, error) {
	...
	if _, err := os.Stat(target); os.IsNotExist(err) { return "", fmt.Errorf("workspace does not exist") }
	return target, nil
}
